...
public class NumberLinkGameManager : MonoBehaviour
{
    public static NumberLinkGameManager Instance;

    [HideInInspector] public bool hasGameFinished;

    public float EdgeSize => _cellGap + _cellSize;

    [SerializeField] private NumberLinkCell _cellPrefab;
    ...
    [SerializeField] private NumberLinkLevelData _levelData;
    [SerializeField] private float _cellGap;
    [SerializeField] private float _cellSize;
    [SerializeField] private float _levelGap;

    private int[,] levelGrid;
    private NumberLinkCell[,] cellGrid;
    private NumberLinkCell startCell;
    ...
    private void Awake()
    {
        ...
        cellGrid = new NumberLinkCell[_levelData.row, _levelData.col];
    ...
    private void SpawnLevel()
    {
        ...
        for (int i = 0; i < _levelData.row; i++)
        {
            for (int j = 0; j < _levelData.col; j++)
            {
                ...
                NumberLinkCell tempCell = Instantiate(_cellPrefab, spawnPos, Quaternion.identity);
    ...
    private void Update()
    {
        ...
        else if (Input.GetMouseButtonUp(0))
        {
            ...
            Vector2 mousePos2D = new Vector2(mousePos.x, mousePos.y);
            RaycastHit2D hit = Physics2D.Raycast(mousePos2D, Vector2.zero);
            if(hit && hit.collider.TryGetComponent(out NumberLinkCell endCell))
            {
    ...
    public NumberLinkCell GetAdjacentCell(int row, int col, int direction)
    {
...
[Serializable]
public struct NumberLinkLevelData
{
    public int row, col;
